...
// The inner thumbnail/badge/title markup, shared by grid and marquee layouts
function VideoCardInner({ video: v }: { video: Video }) {
  return (
...
// Marquee wrapper: the .video-card chrome around the inner content
function VideoCard({ video }: { video: Video }) {
  return (
    <div className="video-card" style={{ height: '100%' }}>
      <VideoCardInner video={video} />
    </div>
  )
}
...
export default function VideoGalleryGrid({ videos }: Props) {
  ...
  const useMarquee = videos.length > 3

  return (
    <>
      {useMarquee ? (
        <div className="reveal">
          <MediaMarquee speed={40}>
            {[...videos, ...videos].map((v, i) => (
              <div
                key={`${v.id}-${i}`}
                className="media-marquee-item video"
                onClick={() => setActiveIndex(i % videos.length)}
              >
                <VideoCard video={v} />
              </div>
            ))}
